...
rurl <- "https://www.imdb.com/title/tt0373889/reviews?ref_=tt_ql_3"
...
write.table(IMDB_reviews, "Harry_Potter_&_Order of Phoenix_Reviews.txt", row.names = F)
...
rev.corpus1 <- tm_map(rev.corpus1, removeWords, c(stpwrd,"the","makes"))
...
wordcloud(words = names(rs1), freq = rs1, random.order = F, colors = brewer.pal(10,"Dark2"), scale = c(2,1), rot.per = 0.2)
...
#Here we can see that this Movie has Almost more Positive reviews than Negative Reviews, But the difference is not much.

...
#I am going to Extract the Reviews of "OnePlus 7T (Glacier Blue, 8GB RAM, Fluid AMOLED Display, 128GB Storage, 3800mAH Battery)" from amazon website
rurl <- "https://www.amazon.in/dp/B07DJHXTLJ#customerReviews"
...
write.table(amazon_reviews, file = "one plus reviews.txt", row.names = F)
...
review.corpus1 <- tm_map(review.corpus1, stripWhitespace) #Removes the extras WhiteSpaces in the Vector

#Lets Form the Term Document Matrix
tdm <- TermDocumentMatrix(review.corpus1)
dtm <- t(tdm) #Converting the Term Document Matrix into Document Term Matrix using Transponse Function
tdm <- as.matrix(tdm)
...
#The Word battery is used the most in the Reviews
...
windows()
wordcloud(words = names(rs_sub), freq = rs_sub, colors = brewer.pal(10, 'Paired')) #This Creates a Wordcloud only with the rs_sub values
#In this WordCloud, the Terms with large Text are the most used words in the Reviews

#Lets Create a WordCloud COnsidering all the Words in the rs
rs1 <- sort(rs, decreasing = T)
wordcloud(words = names(rs1), freq = rs1)

#lets make an Attractive Wordcloud by adding colors to it
wordcloud(words = names(rs1), freq = rs1, random.order = F, colors = brewer.pal(10, 'Set2'), scale = c(2,1), rot.per = 0.2)
...
windows()
wordcloud(words = pos.match.words, freq = pos.freq, colors = rainbow(10), scale = c(3,1))
...
#Here the loud is used in most of the Reviews
...
plot(ft_values, type ="l", main ="One Plus 7T Reviews using Transformed Values", xlab = "Narrative Time", ylab = "Emotional Valence", col = "blue")
...
#So by looking at this Plot we can conclude that this Product has more Positive Reviews so a one can Think of buying it.
...
corpus_clean <- tm_map(sms_corpus, tolower)   #is done to transform all words into lower case are R is Case- Sensitive 
corpus_clean <- tm_map(corpus_clean, removeNumbers) #to remove all the numbers
corpus_clean <- tm_map(corpus_clean, removeWords, stopwords()) #To remove Stopwords means the words which are very common in sentences
corpus_clean <- tm_map(corpus_clean, removePunctuation) #Removes Punctuations
...
sms_dtm <- DocumentTermMatrix(corpus_clean) #Creates a matrix from those clean data
sms_dtm
...
wordcloud(sms_corpus_train, min.freq = 20,random.order = FALSE, max.words = 100,colors = brewer.pal(8, 'Dark2'))

# subset the training data into spam and ham groups
spam <- subset(sms_raw_train, type == "spam")
ham  <- subset(sms_raw_train, type == "ham")

wordcloud(spam$text, max.words = 50, scale = c(3, 0.5),colors = 'red3')
wordcloud(ham$text, max.words = 40, scale = c(3, 1), colors = 'green4')
